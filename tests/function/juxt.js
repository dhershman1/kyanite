...
test('juxt -- Works with no functions and no values', t => {
  t.deepEqual(juxt()(), [])
  t.end()
})

test('juxt -- Works with no functions and some values', t => {
  t.deepEqual(juxt()(2, 3), [])
  t.end()
})

test('juxt -- Works with a single function', t => {
...
test('juxt -- Works with multiple functions and multiple value', t => {
  const add = (x, y) => x + y
